...
function SearchPage() {
  ...
  function deleteProducts(id) {
      ...
      JSON.parse(localStorage.getItem('user')).email!=='admin@test.com' && JSON.parse(localStorage.getItem('user')).password!=='admin' && navigate('/')
  ...
  return (
    <div className="searchPage">
      <div className="searchPage__info">
        <input type="text" placeholder='Search Packages' className='search-product-box ' onChange={(event) => { search(event.target.value.toLowerCase()) }} />
      ...
      </div>
      {
        state.length > 0 ? state.map((item, i) =>
          ...
          <div className="flex" >
           ...
           {JSON.parse(localStorage.getItem('user')).email=='admin@test.com' && JSON.parse(localStorage.getItem('user')).password=='admin' && <Link to={"/update/" + item._id} className="mr-[6px]"><FontAwesomeIcon icon={faPenToSquare} color='orange' /></Link>}
            
            {JSON.parse(localStorage.getItem('user')).email=='admin@test.com' && JSON.parse(localStorage.getItem('user')).password=='admin' &&<FontAwesomeIcon icon={faTrash} color='red' onClick={() => deleteProducts(item._id)} className='  ' />}
            </div>
          </div>
