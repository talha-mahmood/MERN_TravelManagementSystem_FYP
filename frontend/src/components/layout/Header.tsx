  ...
  return (
    <header className="sticky top-0 z-10 bg-white w-full ">
      <div className="bg-white">
        <div className="flex justify-between mx-auto py-0  ml-2 mr-[12px]">
          <div className="mt-6 ">
          ...
          <nav className="hidden md:flex space-x-8 font-semibold my-6 ">
            ...
            {localStorage.getItem('user') ? <>
              ...
              {JSON.parse(localStorage.getItem('user')).email == 'admin@test.com' && JSON.parse(localStorage.getItem('user')).password == 'admin' && <Link to='/add-package' className='mt-1' >
                Add Package
              </Link>}
              ...
              <Link to='/' className='flex gap-1'>
                <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-circle-user-round"><path d="M18 20a6 6 0 0 0-12 0" /><circle cx="12" cy="10" r="4" /><circle cx="12" cy="12" r="10" /></svg>
          ...
          <nav className="md:hidden bg-white py-4 px-8 space-y-4 absolute top-20 left-[100px] right-0 shadow-2xl rounded-xl justify-center">
            ...
            {localStorage.getItem('user') ? <>
              ...
              <Link to='/' className='flex gap-1 hover:bg-secondarycolor hover:text-white'>
                <svg xmlns="http://www.w3.org/2000/svg" className="hover:text-white bg-white" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="lucide lucide-circle-user-round"><path d="M18 20a6 6 0 0 0-12 0" /><circle cx="12" cy="10" r="4" /><circle cx="12" cy="12" r="10" /></svg>

                <span className='mt-1 font-bold '>  {"( " + user ? user.name : '' + " )"} </span>
              </Link>
              {JSON.parse(localStorage.getItem('user')).email == 'admin@test.com' && JSON.parse(localStorage.getItem('user')).password == 'admin' && <Link to='/add-package' className='mt-1 block hover:bg-secondarycolor hover:text-white font-bold' >
                Add Package
              </Link>}
